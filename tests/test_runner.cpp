...
bool test_runner::run_tests( const std::string& prefix, const test_runner::Tests& tests, OutputStyle output_style)
{
    ...
    for (auto t : tests)
    {
        ...
        if (output_style == OutputStyle::ALL)
        {
            if ( test_result )
            {
                std::cout << "\tPASSED: "<< t.description << " ( " 
                         // << chrono_helper::format_duration(elapsed) 
                         << " )" 
                         <<  std::endl;
            }
            else 
            {
                std::cerr << "\tFAILED: "<< t.description << " ( " 
                          // << chrono_helper::format_duration(elapsed) 
                          << " )" 
                          << std::endl;
            }
    ...
    if (output_style != OutputStyle::NONE)
    {
        std::cout << "FINISHED: " << prefix << " Tests: " << tests.size() << " Failing: " << failed_tests <<  " ( " 
                //   << chrono_helper::format_duration(suite_elapsed) 
                  << " )" 
                  << std::endl;
    }
